...
class SQAC24:
    ...
    near dependency ``1 ~= x^2 + y^2 + z^2`` on the gravity sphere.  One
    empirically selected SQAC cross-quadratic term is then added per output.
    The fixed mapping was selected from fit poses only so every DUT uses the
    same equations and needs no per-unit feature-selector metadata.
    """
