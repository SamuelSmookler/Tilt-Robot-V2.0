...
def sweep_for_window(motion, tilt_fast, axis, targets=("x", "y"),
    ...
    for direction in (+1, -1):
        ...
        while walked < RAIL_SEARCH_MAX_DEG:
            ...
            for ch in targets:
                if is_recovered(now[ch]):
                    log(f"  {axis}: '{ch}' entered the +/-"
                        f"{RAIL_RECOVERY_LIMIT_DEG:g} deg recovery band at "
    ...
    motion.move(axis, start)
    span = {c: _span(seen[c]) for c in ("x", "y")}
    log(f"  {axis}: swept +/-{RAIL_SEARCH_MAX_DEG:g} deg, nothing entered the "
        f"+/-{RAIL_RECOVERY_LIMIT_DEG:g} deg recovery band"
...
def auto_level(motion, accel_sense, tilt_all, tilt_fast=None, log=print,
    ...
    # Range discovery deliberately uses the single-reading path, but that path
    # waits RAIL_SEARCH_SETTLE_S after every search move.  Recovery does not
    # stop at the +/-0.45 degree rail boundary: both channels must reach the
    # narrower recovery band so the later +0.10 degree identification move has
    # margin.  The adaptive two-window gate starts after this range recovery.
    now = tilt_fast()
    if now is None:
    ...
    full_spans = None
    for axis in ("OUTER", "INNER"):
        targets = [ch for ch in ("x", "y") if not is_recovered(now[ch])]
        if not targets:
            break
        railed = [ch for ch in targets if is_saturated(now[ch])]
        state = (f"railed channels {railed}" if railed else
                 f"channels {targets} outside the +/-"
                 f"{RAIL_RECOVERY_LIMIT_DEG:g} deg recovery band")
        log(f"  005 {state} (x {now['x']:+.4f}, y {now['y']:+.4f}) -- "
            f"sweeping {axis}; {RAIL_SEARCH_SETTLE_S:g} s settle per move")
        found = sweep_for_window(motion, tilt_fast, axis, targets=targets,
                                 log=log, abort=abort)
        if found["freed"]:
    ...
    recovered = [ch for ch in ("x", "y")
                 if ch not in dead and is_recovered(now[ch])]
    if len(recovered) != 2:
        raise NotInRange(
            f"both 005 channels must be within +/-{RAIL_RECOVERY_LIMIT_DEG:g} "
            f"deg after rail recovery (x {now['x']:+.4f}, y {now['y']:+.4f}, "
            f"recovered {recovered}, live {live}, dead {dead})")
